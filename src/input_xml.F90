module input_xml
  ...
  use ace_header,         only: xs_listing_dict, xs_listings, nuclide_dict, &
                                nuclides, sab_dict, sab_tables, XSListing, &
                                default_expand, n_listings, default_xs, &
                                n_nuclides_total, n_sab_tables, grid_method, &
                                urr_ptables_on
  use cmfd_header,        only: cmfd_run
  use cmfd_input,         only: configure_cmfd
  use constants
  use dict_header,        only: DictIntInt, ElemKeyValueCI
  use error,              only: fatal_error, warning, write_message, message, &
                                verbosity
  use geometry_header,    only: Cell, Surface, Lattice, cells, surfaces, &
                                lattices, universes, n_cells, n_surfaces, &
                                n_lattices, n_universes, overlap_check_cnt, &
                                cell_dict, surface_dict, universe_dict, &
                                lattice_dict, check_overlaps
  use global
  use list_header,        only: ListChar, ListReal
  use material_header,    only: Material, materials, n_materials, material_dict
  use mesh_header,        only: StructuredMesh, meshes, n_meshes, entropy_mesh, &
                                ufs_mesh, source_frac, mesh_dict, &
                                n_cmfd_meshes, n_user_meshes, ufs
  use mpi_interface,      only: master
  use physics,            only: survival_biasing, weight_cutoff, weight_survive
  use plot_header
  use random_lcg,         only: prn, seed
  use source_header,      only: external_source, trace_batch, trace_gen, &
                                trace_particle, trace
